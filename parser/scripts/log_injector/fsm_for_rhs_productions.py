...
def _handled_comment(ch: str, ctx: ParserContext) -> bool:
    if ctx.in_line_comment:  # Handle line-comment.
        ...
        # Condition to exit block-comment.
        elif ch == "/" and ctx.found_asterisk:
            ctx.in_block_comment = False
            ctx.found_backslash = False
            ctx.found_asterisk = False
        else:
            # We could check before, but I chose to just false for optimization.
            ctx.found_asterisk = False
    elif ctx.found_backslash:
        if ch != "/" and ch != "*":
            raise ValueError(
                f"In line {ctx.line_counter}: {ch} followed after '/'. ")
        if ch == "/":
