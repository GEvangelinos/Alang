...
def _write_parsed_stream_to_file(ctx: ParserContext, fout: TextIO) -> None:
        ctx.injectedCharStream.rewind()
        while not ctx.injectedCharStream.eof():
                fout.write(ctx.injectedCharStream.next())


def _copy_section_before_grammar(ctx: ParserContext, fin: TextIO) -> None:
        for line in fin:
...
def _grammar_section_parser(ctx: ParserContext) -> None:
...
def _process_section_inside_grammar(ctx: ParserContext, fin: TextIO) -> None:
        ...
        _grammar_section_parser(ctx)
        ctx.injectedCharStream.extend(line)  # Append line with %% delimiter, as it wasn't appended on stream.


def _copy_section_after_grammar(ctx: ParserContext, fin: TextIO) -> None:
        for line in fin:
                ctx.increment_line_counter()
                ctx.injectedCharStream.extend(line)


def _process_grammar_file(ctx: ParserContext, fin: TextIO) -> None:
        try:
                _copy_section_before_grammar(ctx, fin)
                _process_section_inside_grammar(ctx, fin)
                _copy_section_after_grammar(ctx, fin)
...
def run_parser_pipeline(startup_arguments: StartupArguments) -> None:
        ...
        with open(startup_arguments.input_filename, 'r') as fin:
                _process_grammar_file(ctx, fin)

        with open(startup_arguments.output_filename, 'w') as fout:
                _write_parsed_stream_to_file(ctx, fout)


def launch() -> int:
        try:
                startup_arguments = _parse_startup_arguments()
                _validate_input_file(startup_arguments.input_filename)
