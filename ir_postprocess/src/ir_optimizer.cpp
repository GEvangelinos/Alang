...
bool
IROptimizer::do_jump_threading(ir::QuadStream& quads)
{
    DMASSERT(support::is_in_numeric_range<CodeAddress::UnderlyingType>(quads.size()));
    ...
    for (std::size_t i = 0; i < quads.size(); ++i)
    {
        ...
        CodeAddress jump_target = quads[i].label;
        while (true)
        {
