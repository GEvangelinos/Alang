...
public:
        ...
        static_assert(k_initial_scope_space == 0, "Initial scope space must be 0 to maintain"
                                                  "correct ScopeSpace mapping logic");
        ...
        Variable::ScopeSpace scope_space() const noexcept;
...
public:
        ...
        void append_function_parameter(const std::string &name, Location location);
        const std::list<Parameter> &retrieve_function_parameters();
...
public:
        ...
        ScopeSpaceHandler scope_space_handler;
        ScopeHandler scope_handler;
        ...
        void emit_quad(IOPCode iopcode, const Expr *arg1, const Expr *arg2, const Expr *result,
...
{
        constexpr u32 scope_spaces_for_closure = 2; // 1 formalArg + 1 functionLocal
        ...
        #pragma unroll
        for (int i = 0; i < scope_spaces_for_closure; ++i)
                variable_offset_stack_.pop();
}
// clang-format on

inline Variable::ScopeSpace ScopeSpaceHandler::scope_space() const noexcept
{
        ...
        if (frame_index == k_initial_scope_space)
                return Variable::ScopeSpace::PROGRAM_VAR;
        if (is_odd(frame_index))
                return Variable::ScopeSpace::FORMAL_ARGUMENT;
        return Variable::ScopeSpace::FUNCTION_LOCAL;
}
...
inline void FunctionCtxHandler::enter_function(const std::string &function_name,
...
{
        // Constructor had pushed a stackframe for loops that might occur
        // outside functions. So at the end we expect a single frame to
        // exist.
        DEBUG_SMART_ASSERT(frame_stack_.size() == k_global_data_frame_count);
}

inline void FunctionCtxHandler::append_function_parameter(const std::string &name,
                                                          Location location)
{
        function_parameters_.emplace_back(name, location);
}

inline const std::list<Parameter> &FunctionCtxHandler::retrieve_function_parameters()
{
