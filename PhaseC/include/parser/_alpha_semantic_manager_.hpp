...
#include "core/alpha_diagnostics.hpp"      // for CTIssueTracker, Issue
...
{
public:
    SemanticManager(ParseCtx &parse_ctx, SymbolTable &st, CTIssueTracker &et);

    void N(SourceLocation n_loc, const int N_index);
    void M();
    void forHeader__for_lparen_elist_semicolon_m_expr_semicolon(Expr *expr,
    ...
    void methodCallId__methodcall_id(const char *id, SourceLocation id_loc,
    ...
    void returnStmt__return_expr(Expr *expr, SourceLocation returnStmt_loc,
...
inline SemanticManager::SemanticManager(ParseCtx &parse_ctx, SymbolTable &st,
                                        CTIssueTracker &et) : parse_ctx_(parse_ctx), st_(st), et_(et)
{}
                                                          ...
                                                          const SourceLocation keyword_loc)
{
    ...
    et_.report(CTIssue::Type::SEMANTIC, error, keyword_loc);
    return false;
}
                                             ...
                                             const SourceLocation term_loc)
{
    ...
    et_.report(CTIssue::Type::SEMANTIC, error, term_loc);
}
                                                          ...
                                                          const SourceLocation current_function_loc)
{
    ...
    et_.report(CTIssue::Type::SEMANTIC, error, id_loc,
                     std::list<Issue>{
                         {DT::NOTE, note1, current_function_loc},
                         {DT::NOTE, note2, found_symbol->loc} //
                     } //
    );
}
                                                             ...
                                                             const SourceLocation id_loc)
{
    ...
    if (found_symbol->is_function())
    {
        ...
        et_.report(CTIssue::Type::SEMANTIC, error, id_loc, note,
                         found_symbol->loc);
    }
                                                              ...
                                                              const Parameter &parameter)
{
    ...
    if (st_.is_lib_function(parameter.name))
    {
        ...
        et_.report(CTIssue::Type::SEMANTIC, error, parameter.location);
        return true;
    }
    ...
    if (formal_symbol)
    {
        ...
        et_.report(CTIssue::Type::SEMANTIC, error, parameter.location, note,
                         formal_symbol->loc);
        return true;
    }
    return false;
                                                            ...
                                                            const char *const context)
{
    ...
    case ET::CONST_STRING:
        et_.report(CTIssue::Type::SEMANTIC,
                         FMT::format("{} {}", "Invalid arithmetic expr: ",
                                     context), // TODO: fix ugly AF!
                         expr_loc);
        break;
    default:
        break; // No issue to report for the other Expr Types.
    }
}

inline void SemanticManager::multiStmt__stmt() { parse_ctx_.name_generator.reset_temp_names(); }
    ...
    if (lvalue->type == Expr::Type::TABLE_ITEM)
    {
        ...
        parse_ctx_.quad_handler.emit_quad(
            ...
            parse_ctx_.expr_handler.make_expr_const_int(
                1,
                term_loc), // TODO, Use a global const_int expr (with value 1) Dont
...
inline void SemanticManager::term__lvalue_inc(Expr *&term, Expr *lvalue,
                                              const SourceLocation term_loc)
{
    term__lvalue_op("increment", lvalue, term_loc);
    ...
    else
    {
        ...
        parse_ctx_.quad_handler.emit_quad(
            IOPCode::ADD, lvalue,
            parse_ctx_.expr_handler.make_expr_const_int(1, term_loc), lvalue, term_loc);
    }
}

inline void SemanticManager::term__dec_lvalue(Expr *&term, Expr *lvalue,
                                              const SourceLocation term_loc)
{
...
inline void SemanticManager::term__lvalue_dec(Expr *&term, Expr *lvalue,
                                              const SourceLocation term_loc)
{
    term__lvalue_op("decrement", lvalue, term_loc);
    report_error_if_not_arithmetic(lvalue, term_loc,
...
inline void SemanticManager::lvalue__id(Expr *&lvalue, const char *id_name,
                                        const SourceLocation id_loc)
{
    ...
    lvalue = parse_ctx_.expr_handler.make_expr_variable(
        symbol, symbol->loc); // TODO : PROBABLY not symbol's loc
}
                                               ...
                                               const SourceLocation id_loc)
{
    ...
    // TODO: Or disable quad emission all together even if a single issue is found. (?)
...
inline void SemanticManager::methodCallId__methodcall_id(const char *id,
                                                         const SourceLocation id_loc,
...
inline void SemanticManager::funcPrefix__function_id(const char *id_name,
                                                     const SourceLocation id_loc)
{
    // Update ParseCache
...
inline void SemanticManager::funcSignature__funcPrefix_funcArgList(const Function *&funcSignature)
{
    ...
    // as now we jump for no reason. But thenwe throw issue.
    ...
    if (!!fbi.function_symbol)
    {
        ...
        parse_ctx_.quad_handler.emit_quad(
            ...
            parse_ctx_.expr_handler.make_expr_variable(
                fbi.function_symbol, k_no_location), // TODO: what loc here?
            block_loc.end);
    }
...
inline void SemanticManager::ifPrefix__if_lparen_expr_rparen(Expr *expr,
                                                             const SourceLocation expr_loc)
{
...
inline void
SemanticManager::forHeader__for_lparen_elist_semicolon_m_expr_semicolon(Expr *expr,
...
inline void SemanticManager::funcCtrlStmt__return(const SourceLocation return_loc)
{
    ...
    et_.report(CTIssue::Type::SEMANTIC, error, return_loc);
...
inline void SemanticManager::returnStmt__return(SourceLocation returnStmt_loc,
                                                SourceLocation return_loc)
{
    // TODO functionize like you will do with continue and return...
    if (parse_ctx_.function_ctx_handler.function_nesting_depth() > 0)
                                                     ...
                                                     SourceLocation return_loc)
{
    ...
    if (parse_ctx_.function_ctx_handler.function_nesting_depth() > 0)
    {
        parse_ctx_.quad_handler.emit_quad(IOPCode::RETURN, expr,
                                          nullptr, nullptr,
                                          returnStmt_loc);
...
inline void SemanticManager::backpatch_bool_expr(Expr *expr, SourceLocation expr_loc)
{
    ...
    qh.patch_list(expr->backpatch_lists->true_list, qh.next_quad_label());
    ...
    qh.patch_list(expr->backpatch_lists->false_list, qh.next_quad_label());
