...
template <unsigned column, unsigned column_width, typename T> std::string color_column(T &&value);
...
{
        SMART_ASSERT(!!e);
        auto has_loc = [](const Alpha::Expr *e) -> bool {
                return e->location != Alpha::k_no_location;
        };
        ...
        // clang-format off
                ...
                case AET::BOOLEAN_EXPR:     CASE_ASSERT( has_symbol(e),  has_loc(e), !has_index(e));
                ...
                case AET::CONST_REAL:       CASE_ASSERT(!has_symbol(e),  has_loc(e)  /* ANY_VAL */); 
...
std::string expr_printer(const Alpha::Expr *e)
{
        ...
        // clang-format off
                switch (e->type)
                {
                case AET::ARITHMETIC_EXPR:  return e->symbol->name;
                case AET::ASSIGN_EXPR:      return e->symbol->name;
                case AET::BOOLEAN_EXPR:     return e->symbol->name;
                case AET::CONST_BOOL:       return e->const_bool ? "true" : "false";
                ...
                case AET::LIBRARY_FUNCTION: return e->symbol->name;
...
template <unsigned column, unsigned column_width, typename T> std::string color_column(T &&value)
{
        ...
        // clang-format on
        return FMT::format("{}{:<{}}{}", ascii_color, std::forward<T>(value), column_width,
                           SGR_RESET);
}
...
void Driver::export_compile_errors_impl()
{
        ...
        if (!outfile)
                throw std::runtime_error(
                    FMT::format("Failed opening file {} to export compile errors", outfile_name));
